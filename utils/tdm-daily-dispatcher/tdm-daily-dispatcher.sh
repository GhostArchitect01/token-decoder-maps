...
_log() {
    local message="$1"
    local log_entry="$(date +'%Y-%m-%d %H:%M:%S') [dispatcher v${SCRIPT_VERSION}] $message"
    echo "$log_entry"
    echo "$log_entry" >> /data/data/com.termux/files/home/tdm-daily-dispatcher/dispatcher.log
}
